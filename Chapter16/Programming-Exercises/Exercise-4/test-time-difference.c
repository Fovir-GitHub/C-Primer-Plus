...
// clock() by CLOCKS_PER_SEC gives you the number of seconds elapsed between the two calls.
// Typecasting the values to  double before division enables you to get fractions
// of a second.Write a function that takes a  double argument representing a
// desired time delay and then runs a loop until that amount of time has passed.
...
int main(void) {
	...
	while (scanf("%llu", &loop_time) == 1) {
		...
		delay((double)loop_time / 1000000);  // Assuming loop_time is in microseconds

		clock_t next_time = clock();
		double time_difference = (double)(next_time - first_time) / CLOCKS_PER_SEC;
