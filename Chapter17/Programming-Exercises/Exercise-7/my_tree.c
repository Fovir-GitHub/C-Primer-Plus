...
{
	int compare = strcmp((original_node->node_item).word,
		(new_node->node_item).word);
...
bool add_item_to_tree(item new_item, tree * ptree)
{
	...
	else
	{
		...
		{
			new_node_position =
				where_to_add_new_node(*father_node, new_node);
...
void show_tree_items(node * tree_root)
{
	...
	printf("\"%s\" occurs %d times\n",
		(tree_root->node_item).word, (tree_root->node_item).count);
